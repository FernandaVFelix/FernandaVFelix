...
export const Section = ({ title, items, variant = "grid" }: ISectionProps) => {
  ...
  const handleSetScroll = (scroll: number) => {
    const currrentScrollLeft = scrollRef.current?.scrollLeft || 0;
    scrollRef.current?.scrollTo({
      behavior: "smooth",
      left: currrentScrollLeft + scroll,
    });
  };
      ...
      >
        {variant === "h-list" && (
          <button
            ...
            onClick={() => handleSetScroll(-100)}
            className="h-14 w-14 bg-primary rounded-full hidden sm:flex items-center justify-center sticky my-auto left-0 -ml-14 transition-opacity
           disabled:opacity-0 active:opacity-80"
          >
